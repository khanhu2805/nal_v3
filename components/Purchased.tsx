            ...
            {product_arr.map((product, i) => (
                    <div key={i} className='flex relative items-center mt-20 w-8/12 mx-auto shadow-xl p-5 rounded-xl border-[1px]'>
                        ...
                        <div className='absolute right-10'>
                            <AddCart productId={cloneProduct(product.id, purchasedProduct[i].size, purchasedProduct[i].color, purchasedProduct[i].quanity).productId} selectedSize={cloneProduct(product.id, purchasedProduct[i].size, purchasedProduct[i].color, purchasedProduct[i].quanity).selectedSize} selectedColor={cloneProduct(product.id, purchasedProduct[i].size, purchasedProduct[i].color, purchasedProduct[i].quanity).selectedSize} quanity={cloneProduct(product.id, purchasedProduct[i].size, purchasedProduct[i].color, purchasedProduct[i].quanity).quantity} text="MUA LẠI"></AddCart>
                        </div>
                    </div>     
            ))}
        </div>
